...
import { getPendingOrderCounts, getStoreProducts } from "@/app/lib/queries";

async function StoreProductsPage({ params }: { params: { vendorId: string } }) {
  const session = await auth();
  const isOwner = +params.vendorId === session.user.store_id;

  const result = await DB.query(getStoreProducts(+params.vendorId, isOwner));


  const canDelete = (
    await DB.query(getPendingOrderCounts(+params.vendorId))
  ).rows;
