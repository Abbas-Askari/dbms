    ...
    return (
        <div className="flex justify-center items-center flex-1">
            <form className="flex flex-col gap-4 bg-white shadow-md min-w-[25%] rounded-xl p-12" action={() => role === "Customer" ? makeCustomer({email, phoneNumber, password, firstName, lastName}) : console.log("Making Shop")}>
                ...
                <button tabIndex={0} type="button"  className="btn" onClick={() => setPage(1)}>Next</button>
                </>}
                ...
                <button type="button"  className="btn flex-1" onClick={() => setPage(0)}>Previous</button>
                <button type="button"  className="btn flex-1" onClick={() => {if (verifyEmail(email)) setPage(2)}}>Next</button>
                </div>
                </>}
                ...
                <div className="flex flex-col gap-2">
                <div className="flex flex-col gap-2">
                        <label className="block text-gray-700 text-lg font-bold" htmlFor="first_name">
                            First Name
                        </label>
                        <input
                            className="form-input flex-1"
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="block text-gray-700 text-lg font-bold" htmlFor="last_name">
                            Last Name
                        </label>
                        <input
                            className="form-input flex-1"
                ...
                {error && <p className="text-red-500 underline text-sm">{error}</p>}
                <div className="flex gap-2">
                <button type="button"  className="btn flex-1" onClick={() => setPage(1)}>Previous</button>
                <button type="button"  className="btn flex-1" onClick={() => {if (isFilled(firstName, lastName)) setPage(3)}}>Next</button>
                </div>
                </>}
                ...
                <div className="flex flex-col gap-2">
                <div className="flex flex-col gap-2">
                        <label className="block text-gray-700 text-lg font-bold" htmlFor="phone">
                            Phone
                        </label>
                        <input
                            className="form-input flex-1"
